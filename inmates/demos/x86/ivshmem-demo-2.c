...
static char str[32] = "Hello From IVSHMEM-2 ";
static int ndevices;
...
static void map_shmem_and_bars(struct ivshmem_dev_data *d)
{
	...
	if (cap < 0) {
		printk("		IVSHMEM-2 ERROR: device is not MSI-X capable\n");
		return;
	}
...
{
	printk("		IVSHMEM-2: %02x:%02x.%x sending IRQ\n",
...
{
	printk("		IVSHMEM-2: got interrupt ... %d\n", irq_counter++);
}
...
again:
	...
	if (bdf != -1) {
		printk("		IVSHMEM-2: Found %04x:%04x at %02x:%02x.%x\n",
		...
		printk("		IVSHMEM-2: mapped the bars got position %d\n",
			get_ivpos(d));
	...
	if (!ndevices) {
		printk("		IVSHMEM-2: No PCI devices found .. nothing to do.\n");
		goto out;
	}
